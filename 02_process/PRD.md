...
| 视图选项 | ☰ | 排序方式（手动/最近），选择即持久化到 `dsh.worktable.view.v1`；**默认「手动」**（v2 起；最近为可选——点击即置顶的体验取舍）；另含「管理项目…」入口（§5.5）。分组方式已按用户定案取消 |
...
- **拓扑预设六个**（2 行 × 3 列缩略图网格，聊天窗蓝色 💬 标注）：
  左右两栏 / 三栏横排 / 左一右二 / 上一下二 / 井字四栏 / 左品右聊（2026-08-16 定案）；
  左一右二 = 左列整高内容窗 + 右列「上内容 + 下聊天」（左列布局聊天固定右下，无 ⇄ 翻转）；
