...
· 自定义背景：支持照片与视频统一入库管理——可上传多张媒体、缩略图带类型角标、任意一张可单独删除；每张媒体会独立记住你的调色设置，选中即自动恢复；上传的文件以原始画质存储（不再压缩降清晰度）；视频作为背景时自动循环播放，并加入首尾帧渐变交接，消除循环时的顿挫感。另有：背景网格线开关与透明度滑杆、卡片贴片模糊滑杆；所有滑杆悬停时会显示各自的功能说明。
